    ...
    def fill_box(self, player, cell_1, cell_2):
        ...
        if cell_1_point_1 == cell_2_point_1:
            ...
            if self.square_above:
                ...
                if player == "S":
                    self.score_player_A += 0.5
                    self.score_player_B += 0.5
            pass
...
class Player:
    def __init__(self, player, gameboard):
        ...
        "Last played computer cells for synchronous mode"
        self.last_computer_cell_1 = None
        self.last_computer_cell_2 = None
...
class Synchronous(Player):
    
    def __init__(self, gameboard):
        super().__init__(player="", gameboard=gameboard)

    def move_synchronous(self, player_cell_1, player_cell_2):
        ...
        for p1 in possible_moves:
            for p2 in possible_moves:
                if (
                        (board_class.cells_are_empty(cell_1=p1, cell_2=p2)) and
        ...
        "Select move"
        try:
            computer_cell_1 = best_moves[0][0]
            computer_cell_2 = best_moves[0][1]
        except IndexError:
            random_play = random.choice(allowed_moves)
            computer_cell_1 = random_play[0]
            computer_cell_2 = random_play[1]

        "Formats computer cells"
        computer_cell_1_point_1 = computer_cell_1[0] * 2
        computer_cell_1_point_2 = computer_cell_1[1] * 2
        computer_cell_2_point_1 = computer_cell_2[0] * 2
        computer_cell_2_point_2 = computer_cell_2[1] * 2

        "Player selects a move"
        if (
                (player_cell_1 == computer_cell_1 or player_cell_1 == computer_cell_2) and
                (player_cell_2 == computer_cell_1 or player_cell_2 == computer_cell_2) and
                (board_class.check_box(player_cell_1, player_cell_2) == True)
        ):
            "If the move is the same as the PC (which is always valid) and it fills a box, then point is split (S) "
            board_class.fill_box(player="S", cell_1=player_cell_1, cell_2=player_cell_2)

            "Draws a line"
            if computer_cell_1_point_1 == computer_cell_2_point_1:
                board[computer_cell_1_point_1][(max(computer_cell_1_point_2, computer_cell_2_point_2) - 1)] = self.h_line
            if computer_cell_1_point_2 == computer_cell_2_point_2:
                board[max(computer_cell_1_point_1, computer_cell_2_point_1) - 1][computer_cell_1_point_2] = self.v_line

            "Counts the play"
            board_class.count_play()
            return True
        elif (
                (player_cell_1 == computer_cell_1 or player_cell_1 == computer_cell_2) and
                (player_cell_2 == computer_cell_1 or player_cell_2 == computer_cell_2) and
            ...
            "If the move is the same as the PC and it does not fill a box, returns false to main"
            print("The computer picked the same line but it doesn't fill a box! Try again.")
            return False
        else:
            "If neither case is true, computer and player move independently and player must respect rules"
            if board_class.check_rules(player_cell_1, player_cell_2) == True:

                "Computer makes the move"
                "logs the move"
                board[computer_cell_1_point_1][computer_cell_1_point_2] = self.play
                board[computer_cell_2_point_1][computer_cell_2_point_2] = self.play
                ...
                if board_class.check_box(cell_1=computer_cell_1, cell_2=computer_cell_2):
                    board_class.fill_box(player="B", cell_1=computer_cell_1, cell_2=computer_cell_2)
                ...
                "Formats cell 1"
                player_cell_1_point_1 = player_cell_1[0] * 2
                player_cell_1_point_2 = player_cell_1[1] * 2

                "Formats cell 2"
                player_cell_2_point_1 = player_cell_2[0] * 2
                player_cell_2_point_2 = player_cell_2[1] * 2

                "logs the move"
                board[player_cell_1_point_1][player_cell_1_point_2] = self.play
                board[player_cell_2_point_1][player_cell_2_point_2] = self.play

                "draws line between cells"
                "if cells are horizontally adjacent draw an horizontal line"
                if player_cell_1_point_1 == player_cell_2_point_1:
                    board[player_cell_1_point_1][(max(player_cell_1_point_2, player_cell_2_point_2) - 1)] = self.h_line

                "if cells are vertically adjacent draw a vertical line"
                if player_cell_1_point_2 == player_cell_2_point_2:
                    board[max(player_cell_1_point_1, player_cell_2_point_1) - 1][player_cell_1_point_2] = self.v_line

                "Check if move draws a box"
                if board_class.check_box(cell_1=player_cell_1, cell_2=player_cell_2):
                    board_class.fill_box(player="A", cell_1=player_cell_1, cell_2=player_cell_2)
                ...
                "If rules are not respected, both player and PC start again. (To avoid PC from logging a move in the board)"
                return False
